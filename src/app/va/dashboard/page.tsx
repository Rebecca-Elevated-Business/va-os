...
export default function TaskCentrePage() {
  ...
  // --- GLOBAL TICKER ---
  useEffect(() => {
    // Let the interval handle the first update to avoid render cascades
    ...
    await supabase.from("tasks").insert([
      {
        ...
        status: newTaskStatus, // Use the dynamic status (e.g. from Kanban column)
  ...
  const updateStatus = async (taskId: string, newStatus: string) => {
    // Optimistic update for drag-and-drop smoothness
    setTasks((prev) =>
  ...
  return (
    <div className="min-h-screen text-black pb-20">
      ...
      {/* 2. FILTERS (Only show on List/Kanban, usually Calendar shows all) */}
